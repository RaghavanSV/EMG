...
class ModifyBinary:
    ...
    def append_benign_data_overlay(self):
        ...
        if random_benign_file is None:
            return self.bytez
        benign_binary = lief.PE.parse(random_benign_file)
        if benign_binary is None:
            return self.bytez
        text_section = benign_binary.get_section(".text")

        if text_section is None:
            return self.bytez  # or log warning, or choose another benign binary

        benign_binary_section_content = text_section.content
    ...
    def add_section_benign_data(self):
        ...
        if text_section is None:
            return self.bytez  # or handle accordingly
    ...
    def add_section_strings(self):
        ...
        section = lief.PE.Section(random.choice(available_section_names))
        section.content = list(good_strings.encode("utf-8", errors="ignore"))
        binary.add_section(section, lief.PE.SECTION_TYPES.DATA)
    ...
    def add_strings_to_overlay(self):
        ...
        """
        good_strings = self._randomly_select_good_strings()
        self.bytez += good_strings.encode("ascii",errors="ignore")
    ...
    def remove_debug(self):
        ...
        if binary.has_debug:
            for i, e in enumerate(binary.data_directories):
                if str(e.type) == "DATA_DIRECTORY.DEBUG":
                    e.rva = 0
                    e.size = 0
                    self.bytez = self._binary_to_bytez(binary)
                    return self.bytez
...
'''
        def xor_packer(self):
                os.system(f"python3 xorpacker.py -f {}")
'''

def modify_sample(bytez, action):
    with open(bytez,'rb') as f:
        dbytez=f.read()
    tbytez = ModifyBinary(dbytez).__getattribute__(action)()
    return tbytez
